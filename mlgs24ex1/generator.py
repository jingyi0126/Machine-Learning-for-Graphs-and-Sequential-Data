...
class HMM_TxtGenerator:
    ...
    def forwards(self, x):
        ...
        alpha[0] = (pi.flatten() * B[:,x[0]]).T

        for t in range(1,T):
            alpha[t] = (B[:,x[t]] * (A.T @ alpha[t-1])).T
        return alpha


    def log_likelihood(self, alpha:np.ndarray):
    ...
    def generate_sentence(self, sentence_length):
        ...
        ### YOUR CODE HERE ###
        A = self.model_params.A #[K*K]
        B = self.model_params.B #[K*V]
        pi = self.model_params.pi #[K*1]

        z_seq = np.zeros(sentence_length).astype(int)
        x_seq = np.zeros(sentence_length).astype(int)
        K = np.arange(A.shape[0])
        V = np.arange(B.shape[1])
        z_seq[0] = np.random.choice(K,size=1,p=pi.flatten())
        x_seq[0] = np.random.choice(V,size=1,p=B[z_seq[0]])
        for t in range(1,sentence_length):
            z_seq[t] = np.random.choice(K,size=1,p=A[z_seq[t-1]])
            x_seq[t] = np.random.choice(V,size=1, p=B[z_seq[t]])
        
        return self.X_to_sentence(x_seq)
